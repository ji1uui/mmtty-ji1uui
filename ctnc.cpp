...
==============================================================
*/
// ISerialDevice::PutChar - 1バイトを送信バッファに積む (型を BYTE に変更)
void CCtnc::PutChar(BYTE c)
{
	if( m_CreateON == TRUE ){
...
// PutRawChar - 旧 PutChar(char) の後方互換ラッパー
void __fastcall CCtnc::PutRawChar(char c)
{
	PutChar((BYTE)c);
}
...
void CCtnc::Write(const void *pData, DWORD len)
{
	if( m_CreateON == TRUE ){
