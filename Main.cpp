	...
	: TForm(Owner)
{
	...
	if( lcid != LANG_JAPANESE ){
		...
		sys.m_WinFontName = "�l�r �o�S�V�b�N";
		sys.m_WinFontCharset = SHIFTJIS_CHARSET;
		sys.m_BtnFontName = "�l�r �S�V�b�N";
		sys.m_BtnFontCharset = SHIFTJIS_CHARSET;
		sys.m_FontName = "�l�r �S�V�b�N";
	...
	sys.m_HelpFontName = "�l�r �S�V�b�N";
...
//	Panel2->Top = GroupBox1->Height + 1;	// ��M��ʃT�C�Y�̒���
	UpdatePanel();
	LogLink.SetHandle(Handle, CM_CMML);

	ReadSampFreq();
	pSound = new TSound(TRUE);
	WriteProfile(1025, "Default", TRUE);				// �f�t�H���g�p�����[�^�̋L��
	ReadRegister();
	WriteProfile(1026, "Return to the startup", TRUE);	// �f�t�H���g�p�����[�^�̋L��
...
//�f�[�^��5�ƃX�g�b�v �r�b�g2�̑g�ݍ��킹�͖����ł��B
//�f�[�^��6� 7� 8�ƃX�g�b�v �r�b�g1.5�Ƃ̑g�ݍ��킹�������ł��B
...
void __fastcall TMmttyWd::OpenClosePTT(void)
{
	...
	if( !strcmp(sys.m_TxRxName.c_str(), "NONE") ){
		...
		if( pComm->Open(ComPort, sys.m_TxRxInv, sys.m_TxPort ? &cm : NULL) ){
			...
			if( sys.m_TxPort ) pComm->Resume();		// Txd���M�^�X�N�𓮍삳����
		}
		else {
			...
			if( !Remote || !m_RemoteTimer ){
				if (cm.Baud<110){ //1.70F notify user if attempting to open port at low speed, as many devices can't do this
					ErrorMB( (Font->Charset != SHIFTJIS_CHARSET)? "Cannot open '%s' at '%s' baud; if the selected serial port can't handle this baud rate, use EXTFSK.":"'%s'���{�[���[�g'%s'�ŊJ���܂���.�����A�I�������V���A���|�[�g�����̃{�[���[�g���T�|�[�g���Ă��Ȃ��Ƃ���EXTFSK���g���Ă��������B", sys.m_TxRxName.c_str(),AnsiString(cm.Baud).c_str());
				}
				else {
					ErrorMB( (Font->Charset != SHIFTJIS_CHARSET)? "Cannot open '%s'":"'%s'���I�[�v���ł��܂���.", sys.m_TxRxName.c_str());
				}
			}
		}
_noerr:;
...
//AA6YQ option, Added by JE3HHT on Sep.2010
	...
	delete pIniFile;
		...
		ErrorMB((Font->Charset != SHIFTJIS_CHARSET)?"Cannot update MMTTY.INI":"MMTTY.INI���X�V�ł��܂���.");
	}
}
//---------------------------------------------------------------------------
...
_try:;
	...
	if( m_PaletteTimer ){				// 256�F�r�f�I�̎�
		m_PaletteTimer--;
		if( !m_PaletteTimer ){
			PBoxWater->Invalidate();
			PBoxXY->Invalidate();
	...
	if( m_InitFirst == TRUE ){
		...
		if( !Remote ){
			if( (sys.m_Call == "NOCALL") || sys.m_Call.IsEmpty() ){
				...
				if( InputMB("MMTTY", (Font->Charset != SHIFTJIS_CHARSET)?"Please enter your callsign":"���Ȃ��̃R�[���T�C������͂��ĉ�����.", as) == TRUE ){
					if( !as.IsEmpty() ){
						char bf[MLCALL+1];
		...
		if( !Remote ){
			...
			Log.Open(NULL, !f1stInst);		// �J�����g���O�t�@�C���̃I�[�v��
			LogLink.UpdateLink(sys.m_LogLink);
		}
		UpdateTextData();
...
void __fastcall TMmttyWd::ToRX(int sw)
{
	if( SBTX->Down ){
		...
		pSound->FSKMOD.SetDiddleTimer(-1);		// Diddle �֎~
		pSound->TrigBCC();
		pSound->FSKMOD.DeleteTXBuf();
		SBTX->Caption = "REQ";
		SBTX->Update();
...
// ���[�U�[��`�\�ȃL�[
		...
		for( i = 0; i < MSGLISTMAX; i++ ){		// ���b�Z�[�W���X�g
...
void __fastcall TMmttyWd::FindCall(void)
{
	...
	if( Log.FindSet(&Log.m_Find, Log.m_sd.call) ){
		Log.SetFreq(&Log.m_sd, AnsiString(Freq->Text).c_str());	//JA7UDE 0428
		SDMMLOG	sd;
		Log.GetData(&sd, Log.m_Find.pFindTbl[0]);	// ��ԍŐV�̃f�[�^
	...
	TShiftState Shift, int X, int Y)
{
	...
	if( bf[0] ){
		...
		else if( Log.m_LogSet.m_Contest ){		// �R���e�X�g��
			if( (strlen(bf) <= 2) ||
				!IsCall(bf) ||
...
//				DelChar(bf, '/');
			...
			if( (!Log.m_LogSet.m_DefMyRST) && SBQSO->Down && !MyRST->Text.IsEmpty() ){
				if( YesNoMB((Font->Charset != SHIFTJIS_CHARSET)? "Change MyRST ?":"MyRST��ύX���܂���?") != IDYES ) return;
			}
			SetYourRST(bf);
		}
		else if( IsCall(bf) ){
			if( strcmpi(bf, sys.m_Call.c_str()) ){
				if( SBQSO->Down && !HisCall->Text.IsEmpty() ){
					if( YesNoMB((Font->Charset != SHIFTJIS_CHARSET)?"Change Callsign?":"�R�[���T�C����ύX���܂���?") != IDYES ) return;
				}
				SetYourCallsign(bf);
			}
		}
			...
			if( SBQSO->Down && !HisQTH->Text.IsEmpty() ){
				if( YesNoMB((Font->Charset != SHIFTJIS_CHARSET)?"Change QTH?":"�p�s�g��ύX���܂���?") != IDYES ) return;
			}
			SetYourQTH(bf);
		}
		else if( IsName(bf) ){
			if( SBQSO->Down && !HisName->Text.IsEmpty() ){
				if( YesNoMB((Font->Charset != SHIFTJIS_CHARSET)?"Change Name?":"���O��ύX���܂���?") != IDYES ) return;
			}
			SetYourName(bf);
		}
	}
...
#if USEPAL
//---------------------------------------------------------------------------
// ���݂̘_���p���b�g��Ԃ��iTControl::GetPalette�̃I�[�o���C�h�֐��j
...
{
	TEditDlgBox *pBox = new TEditDlgBox(this);
	pBox->Caption = (Font->Charset != SHIFTJIS_CHARSET)? "Edit Button":"���̓{�^���ҏW";
...
//			pSound->m_IDDevice = sys.m_SoundDevice;
		...
		if( sys.m_SampFreq != SampFreq ){
			if( Font->Charset != SHIFTJIS_CHARSET ){
				...
				InfoMB( "�V�����T���v�����O���g����K�p�����邽�߂�%s���ċN�����ĉ�����.", Remote ? "�A�v���P�[�V����":"MMTTY" );
			}
		}
...
int __fastcall TMmttyWd::OutputFile(LPCSTR pName)
{
	...
	if( (fp = fopen(pName, "rt"))!=NULL ){
		...
		ErrorMB((Font->Charset != SHIFTJIS_CHARSET) ? "'%s' was not found":"'%s'��������܂���.", pName);
		return FALSE;
	}
	ToTX(in.c_str());
	OutputStr(in.c_str());
...
void __fastcall TMmttyWd::KFileOutClick(TObject *Sender)
{
	...
	if(Font->Charset != SHIFTJIS_CHARSET){
		...
		OpenDialog->Title = "���M����e�L�X�g�t�@�C�����J��";
		OpenDialog->Filter = "�e�L�X�g�t�@�C��(*.txt)|*.txt|���ׂẴt�@�C��(*.*)|*.*|";
	}
	OpenDialog->FileName = "";
...
void __fastcall TMmttyWd::KSaveRxClick(TObject *Sender)
{
	...
	if( SaveDialog->Execute() == TRUE ){
		...
		if( fp != NULL ){
			fputs(as.c_str(), fp);
			if( fclose(fp) ){
				ErrorMB( (Font->Charset != SHIFTJIS_CHARSET) ? "Write Error to '%s'":"'%s'�ɐ������������߂܂���ł���.", SaveDialog->FileName.c_str());
			}
			else {
			...
			ErrorMB((Font->Charset != SHIFTJIS_CHARSET)?"Write Error to '%s'": "'%s'���쐬�ł��܂���.", SaveDialog->FileName.c_str());
		}
	}
	TopWindow(this);
	DisPaint = FALSE;
...
void __fastcall TMmttyWd::QSOOUT(int sw)
{
	...
	if( !Log.m_sd.ur[0] || !Log.m_sd.my[0] ){	// �����f�[�^
		Log.m_sd.send = 'I';
	}
	Log.PutData(&Log.m_sd, Log.m_CurNo);
...
void __fastcall TMmttyWd::SBInitClick(TObject *Sender)
{
	...
	if( SBQSO->Down ){
		...
		if( Log.m_LogSet.m_Contest ){
			...
			r = YesNoMB( (Font->Charset != SHIFTJIS_CHARSET) ? "Delete on this QSO. Are you sure?":"����QSO�𖳌��ɂ��܂����H" );
		}
		if( r == IDYES ){
			Log.DeleteLast();
			Log.m_Find.Clear();
...
void __fastcall TMmttyWd::KOpenLogClick(TObject *Sender)
{
	...
	if( (Font->Charset != SHIFTJIS_CHARSET) ){
		...
		OpenDialog->Filter = "MMLOG�f�[�^�t�@�C��(*.mdt)|*.mdt|";
	}
	OpenDialog->FileName = "";
...
void __fastcall TMmttyWd::KRxRecClick(TObject *Sender)
{
	if( (Font->Charset != SHIFTJIS_CHARSET) ){
		...
		SaveDialog->Title = "MMTTY�T�E���h�t�@�C���̍쐬";
		SaveDialog->Filter = "MMTTY�T�E���h�t�@�C��(*.mmv)|*.mmv|";
	}
...
void __fastcall TMmttyWd::KRxPlayClick(TObject *Sender)
{
	...
	if( (Font->Charset != SHIFTJIS_CHARSET) ){
		...
		OpenDialog->Title = "MMTTY�T�E���h�t�@�C���̍Đ�";
		OpenDialog->Filter = "MMTTY�T�E���h�t�@�C��(*.mmv)|*.mmv|";
	}
	OpenDialog->FileName = "";
...
void __fastcall TMmttyWd::KExtEntClick(int n, TMenuItem *mp)
{
	...
	if(Font->Charset != SHIFTJIS_CHARSET){
		...
		r = InputMB("�O���v���O�����o�^", "���j���[�ɕ\�����閼�O����͂��ĉ�����.", as);
	}
	if( r == TRUE ){
		OpenDialog->Options >> ofCreatePrompt;
		OpenDialog->Options << ofFileMustExist;
		if( (Font->Charset != SHIFTJIS_CHARSET) ){
			...
			OpenDialog->Title = "���s����v���O������I��";
			OpenDialog->Filter = "�v���O�����t�@�C��(*.exe;*.lnk)|*.exe;*.lnk|���ׂẴt�@�C��(*.*)|*.*|";
		}
...
void __fastcall TMmttyWd::OpenCloseTNC(void)
{
	...
	if( strcmp(TNC.StrPort, "NONE" ) ){
		...
		if( pTnc->m_CreateON != TRUE ){
			delete pTnc;
			pTnc = NULL;
			ErrorMB( (Font->Charset != SHIFTJIS_CHARSET)? "Cannot open '%s' for TNC":"TNC�͋[�|�[�g'%s'���I�[�v���ł��܂���.", TNC.StrPort);
		}
		else {
			sys.m_DisTX &= ~1;
			KTNC->Checked = TRUE;
...
void __fastcall TMmttyWd::RemoteMMTTY(tagMSG &Msg)
{
	...
	switch(Msg.wParam){
		case RXM_HANDLE:		// �E�C���h�E�n���h���̒ʒm
			m_RemoteTimer = 0;
			...
			break;
		case RXM_REQHANDLE:		// �E�C���h�E�n���h���̗v��
			::PostMessage(APP_HANDLE, MSG_MMTTY, TXM_HANDLE, (DWORD)Handle);
			break;
		case RXM_PTT:			// ����̐؂�ւ�
			switch(Msg.lParam){
...
void __fastcall TMmttyWd::OpenCloseRadio(void)
{
	...
	if( strcmp(RADIO.StrPort, "NONE" ) ){
		...
		if( pRadio->m_CreateON != TRUE ){
			...
			pRadio = NULL;
			if( !Remote || !m_RemoteTimer ){
				ErrorMB( (Font->Charset != SHIFTJIS_CHARSET)? "Can't open '%s' for Radio commands":"Radio�R�}���h�|�[�g'%s'���I�[�v���ł��܂���.", RADIO.StrPort);
			}
		}
		else {
...
//AA6YQ option, Added by JE3HHT on Sep.2010
	...
	if( Remote ) PostApp(TXM_PROFILE, (Flag ? 0x00010000 : 0x00020000) + n);
		...
		ErrorMB((Font->Charset != SHIFTJIS_CHARSET)?"Cannot update UserPara.INI":"UserPara.INI���X�V�ł��܂���.");
	}
}
//---------------------------------------------------------------------------
void __fastcall TMmttyWd::WriteProfile(int n)
{
	...
	if( Font->Charset != SHIFTJIS_CHARSET ){
		...
		r = InputMB("�v���t�@�C���̓o�^", "�v���t�@�C���̖��O", as);
	}
	if( r == TRUE ){
		if( tp != NULL ){
			tp->Caption = as;
...
void __fastcall TMmttyWd::KSSVClick(TObject *Sender)
{
	if( (Font->Charset != SHIFTJIS_CHARSET) ){
		...
		SaveDialog->Title = "�v���t�@�C���̍쐬";
		SaveDialog->Filter = "�v���t�@�C��(*.pro)|*.pro|";
	}
	char bf[128];
...
void __fastcall TMmttyWd::KSLDClick(TObject *Sender)
{
	...
	if(Font->Charset != SHIFTJIS_CHARSET){
		...
		OpenDialog->Title = "�v���t�@�C���̃��[�h";
		OpenDialog->Filter = "�v���t�@�C��(*.pro)|*.pro|���ׂẴt�@�C��(*.*)|*.*|";
	}
	OpenDialog->FileName = "";
...
void __fastcall TMmttyWd::KPttTimClick(TObject *Sender)
{
	...
	as = m_PttTimer;
	if( InputMB("MMTTY", (Font->Charset != SHIFTJIS_CHARSET)?"Enter PTT timer value(sec). (0 = OFF)":"PTT�^�C�}�[���ԁi�b�j����͂��ĉ�����. (0 = OFF)", as) == TRUE ){
		if( !as.IsEmpty() ){
			int d;
...
void __fastcall TMmttyWd::KROFClick(TObject *Sender)
{
	...
	if( InputMB("MMTTY", (Font->Charset != SHIFTJIS_CHARSET)?"Please enter folder name":"�t�H���_������͂��ĉ�����.", as) == TRUE ){
