...
_try:;
		...
		if( m_CreateON == TRUE ){
			if( !m_TxEnb ){
				...
				if( m_FirstGaurd ){		// �ŏ��̃K�[�h���Ԃ̊m��
					m_FirstGaurd = 0;
					::Sleep(250);
					if( m_DisDiddle == -1 ) m_DisDiddle = 0;
				}
				else if( m_txcnt && (FSKDeff <= DEFFSOUND) ){	// ���M�f�[�^����
					m_DisDiddle = pMod->m_DisDiddle;
...
void __fastcall CComm::Close(void)
{
	if( m_CreateON == TRUE ){
		if( m_Execute ){
			m_TxEnb = 0;
			m_Command = COMM_CLOSE;	// �X���b�h�I���R�}���h
			Priority = tpNormal;		//�X���b�h�͒ʏ�̗D��x�ł���
#if 0
			DWORD tim = GetTickCount();
			while( m_Command && (GetTickCount() < (tim + 3000)) ){			// �X���b�h�I���҂�
				::Sleep(1);
			}
#else
			WaitFor();
...
#if 0
			DWORD tim = GetTickCount();
			while( GetTickCount() < (tim + 3000) ){		// 1[s]�̃E�G�C�g
				if( !m_Command ) break;
				::Sleep(1);
			}
#else
...
//	Application->MainForm->Caption = int(cp ? cp->Stop == TWOSTOPBITS : FALSE);
	...
	m_dcb.fRtsControl = m_inv ? RTS_CONTROL_ENABLE : RTS_CONTROL_DISABLE;		// ���M�֎~
	m_dcb.fDtrControl = m_inv ? DTR_CONTROL_ENABLE : DTR_CONTROL_DISABLE;		// ���M�֎~
...
// Write は PutChar の連続呼び出しとして実装される (SRP: 既存 PutChar の再利用)
