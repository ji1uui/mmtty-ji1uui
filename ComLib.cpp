...
char	LogDir[256];		// ��M�L�^�̃f�B���N�g��
char	OutFileDir[256];	// �e�L�X�g���M�t�@�C���̃f�B�G�N�g��
char	MMLogDir[256];		// MMLOG�t�@�C���̃f�B���N�g��
char	ExtLogDir[256];		// �G�N�X�|�[�g�t�@�C���̃f�B���N�g��
char	RecDir[256];		// �^���t�@�C���̃f�B���N�g��
AnsiString	JanHelp;
...
double	SampFreq = 11025.0;	// �T���v�����O���g��
...
void __fastcall ShowHtmlHelp(LPCSTR pContext)
{
	...
	snprintf(bf, sizeof(bf), "%s%s", BgnDir, sys.m_HTMLHelp.c_str());
	FILE *fp;
	if( (fp = fopen(bf, "rb"))!=NULL ){
		...
		if( !strcmpi(GetEXT(bf), "HLP") ){
			...
			if( pContext ){
				char cmd[512];
                snprintf(cmd, sizeof(cmd), "HH.EXE %s::/%s", bf, pContext);
...
void CPrintText::OpenLogFile(void)
{
	...
	if( m_fp == NULL ){
		ErrorMB( "%s ���I�[�v���ł��܂���.", sys.m_LogName.c_str() );
		return;
	}
	fprintf(m_fp, "\r\n");
}
...
void CPrintText::CloseLogFile(void)
{
	if( m_fp != NULL ){
		if( fclose(m_fp) ){
			m_fp = NULL;
			ErrorMB( "%s ���������쐬�ł��܂���ł���.", sys.m_LogName.c_str() );
		}
		m_fp = NULL;
	}
}
...
void CFifoEdit::ScrollBarChange(void)
{
	...
	if( pScroll->Max == pScroll->Position ){		// �ŏI�s�̎�
		m_DisEvent++;
...
void CFifoEdit::Clear(void)
{
	...
	m_DispTop = 0;								// �\���g�b�v�s�̈ʒu

	m_WriteLine = 0;	// �������ݍs�̈ʒu
	m_WriteCol = 0;		// �������݃J�����ʒu

	m_ReadLine = 0;		// ��肾���s�̈ʒu
	m_ReadCol = 0;		// ��肾���J�����ʒu
...
{
	if( m_ReadLine > m_WriteLine ) return 0;		// ���o�����Ȃ�
	if( m_ReadLine == m_WriteLine ){	// ����s�̎�
		if( m_WriteCol <= m_ReadCol ) return 0; 	// ���o�����Ȃ�
	}
...
{
	if( m_ReadLine > m_WriteLine ) return 1;		// ���o�����Ȃ�
	if( m_ReadLine == m_WriteLine ){	// ����s�̎�
		if( m_WriteCol <= m_ReadCol ) return 1; 	// ���o�����Ȃ�
	}
	if( m_ReadLine < m_WriteLine ) return 1;		// �����s�̂Ƃ�
...
{
	if( m_ReadLine > m_WriteLine ) return FALSE;		// ���o�����Ȃ�
	if( m_ReadLine == m_WriteLine ){	// ����s�̎�
		if( m_WriteCol <= m_ReadCol ) return FALSE; 	// ���o�����Ȃ�
	}
	if( m_WriteCol ){
		m_WriteCol--;
	}
...
void CFifoEdit::PutChar(char c)
{
	...
	if( m_WriteLine >= FIFOLINEMAX ){		// �o�b�t�@�t���̎�
