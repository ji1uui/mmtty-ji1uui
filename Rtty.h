...
#define	BITDEBUG	0		// �L�����N�^���̑���
#define	TXDDEBUG	0		// FSK�̃L�����N�^���̑���
#define	FSKDEBUG	0		// FSK�̃f�o�b�O���[�h
...
private:
	...
	int		Count;		// �f���W�����[�^�f�B�X�p�b�`
	double	i2;			// �f���W�����[�^2ndInput
	...
	// SOLID OCP/SRP: Strategy パターンによる復調アルゴリズムの切り替え。
	// m_type の値に応じた if-else 分岐を廃止し、IDemodStrategy に委譲する。
	// 新しい復調方式は IDemodStrategy を実装するだけで追加できる (OCP)。
	// SetDemodStrategy() で実行時に差し替え可能。
	IDemodStrategy *m_pDemodStrategy;

	// デフォルト戦略オブジェクト (後方互換: m_type で初期選択される)
...
//	int		m_Bit;
	...
	int		m_majority;		// ���������W�b�N
	int		m_ignoreFream;	// �t���[�~���O�G���[�̖���
...
public:
	...
	// 復調戦略を実行時に差し替える (OCP)
	// pStrategy が NULL の場合は m_type に基づくデフォルト戦略を使用する
...
// SOLID SRP: State パターンにより変調器の状態管理を分離。
// m_mode int フラグ + if-else 分岐に代わり、IModulatorState に状態ロジックを委譲。
// 各状態クラス (CModIdleState/CModTransmitState/CModWaitState) が
// 自身の責任のみを担当する (SRP)。
class CFSKMOD
{
...
private:
	...
	// State パターン: 現在の変調状態を保持するオブジェクト
	IModulatorState   *m_pState;

	// 具体状態オブジェクト (インライン保持でヒープ確保を回避)
...
public:
	...
	// State パターン: 状態を遷移させる
	// (Enter/Exit コールバックで各状態の初期化・クリーンアップを行う)
...
class CNoise		// M�n��m�C�Y N=22 (Tap=1)
{
private:
